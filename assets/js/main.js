...
let svgI = svgW / 627; // 190 = viewBox="..X..X.190..X"
...
function openClose() {
   checkIn.style.color = (currentHour >= 15 && currentHour <= 19) ? "green" : "red";
   checkOut.style.color = (currentHour >= 8 && currentHour <= 9) ? "green" : "red";
}
      ...
      if (st > lastScrollTop) {
         ...
         for (let i = 0; i < animateBoxes.length; i++) {
            ...
            if (rect.top > windowHeight + 20) {
               animateBox.classList.remove('animated', 'fadeIn', 'fadeInUp', 'fadeInLeft', 'fadeInRight', 'fadeInRightText', 'fadeInLeftText', 'fadeInScale');
            }
         }
      }
